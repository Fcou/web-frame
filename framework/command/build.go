...
var buildSelfCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		if err != nil {
			log.Fatalln("fcou go: please install go in path first")
		}

		cmd := exec.Command(path, "build", "-o", "fcou", "./")
		...
		fmt.Println("build success please run ./fcou direct")
		return nil
	},
}
...
var buildFrontendCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		// 执行npm run build
		cmd := exec.Command(path, "run", "build", "./fcou")
		// 修改命令的工作目录为./fcou
		cmd.Dir = "./fcou"
