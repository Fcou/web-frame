...
func (p *Proxy) rebuildBackend() error {
	// 重新编译fcou
	cmdBuild := exec.Command("./fcou", "build", "backend")
...
func (p *Proxy) restartBackend() error {
	...
	fcouAddress := fmt.Sprintf(":" + port)
	// 使用命令行启动后端进程
	cmd := exec.Command("./fcou", "app", "start", "--address="+fcouAddress)
...
func (p *Proxy) restartFrontend() error {
	...
	// 就是让启动命令变为  PORT=8071 npm run dev
	cmd.Env = os.Environ()
