...
import (
	...
	"github.com/Fcou/web-frame/framework/provider/log/formatter"
)
...
// logf 为打印日志的核心函数
func (log *FcouLog) logf(level contract.LogLevel, ctx context.Context, msg string, fields map[string]interface{}) error {
	// 先判断日志级别
...
// Panic 输出panic的日志信息
func (log *FcouLog) Panic(ctx context.Context, msg string, fields map[string]interface{}) {
	log.logf(contract.PanicLevel, ctx, msg, fields)
}

// Fatal will add fatal record which contains msg and fields
func (log *FcouLog) Fatal(ctx context.Context, msg string, fields map[string]interface{}) {
	log.logf(contract.FatalLevel, ctx, msg, fields)
}

// Error will add error record which contains msg and fields
func (log *FcouLog) Error(ctx context.Context, msg string, fields map[string]interface{}) {
