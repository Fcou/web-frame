...
import (
	...
	"github.com/Fcou/web-frame/framework/provider/log/services"
)

// FcouLogServiceProvider 服务提供者
type FcouLogServiceProvider struct {
...
// Register 注册一个服务实例
func (l *FcouLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewFcouConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewFcouSingleLog
	case "rotate":
		return services.NewFcouRotateLog
	case "console":
		return services.NewFcouConsoleLog
	case "custom":
		return services.NewFcouCustomLog
	default:
		return services.NewFcouConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *FcouLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *FcouLogServiceProvider) IsDefer() bool {
	return false
}

// Params 定义要传递给实例化方法的参数
func (l *FcouLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
...
// Name 定义对应的服务字符串凭证
func (l *FcouLogServiceProvider) Name() string {
