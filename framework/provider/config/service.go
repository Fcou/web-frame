...
// FcouConfig  表示fcou框架的配置文件服务
type FcouConfig struct {
...
func NewFcouConfig(params ...interface{}) (interface{}, error) {
	...
	for _, file := range files {
		fileName := file.Name()
		err := fcouConf.loadConfigFile(envFolder, fileName)
		if err != nil {
			log.Println(err)
			continue
		}
			...
			case ev := <-watch.Events:
				{
					...
					if ev.Op&fsnotify.Create == fsnotify.Create {
						...
						fcouConf.loadConfigFile(folder, fileName)
					}
					...
					if ev.Op&fsnotify.Remove == fsnotify.Remove {
						...
						fcouConf.removeConfigFile(folder, fileName)
					}
				}
...
// 通过path来获取某个配置项
func (conf *FcouConfig) find(key string) interface{} {
	conf.lock.RLock()
